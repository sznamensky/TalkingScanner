#!/bin/bash
echo Script TalkingScanner3_54R.sh created by Sergey Znamenskiy 10012024 using algorythm Talking Scanner 3.5R
echo changed the scanner mountpoint to /media/..
echo directory for temporary files /tmp/TalkingScanner
echo directory for results file /var/tmp/TalkingScanner
echo "Привет! Я робот Говорилка! Буду озвучивать для вас текст из сканов." |RHVoice-test -p Vsevolod
...
echo "Забираю сканы из сканера"|RHVoice-test -p Vsevolod
mv /media/6335-3931/DCIM/100MEDIA/* /tmp/TalkingScanner
FILENUMBER=`ls /tmp/TalkingScanner |grep IMAG |wc -l`
echo "Загружено $FILENUMBER сканов из сканера"|RHVoice-test -p Vsevolod
echo Checkpoint 7
...
echo Checkpoint 13
var13=`ls /tmp/TalkingScanner |grep scanneron.txt`
echo "Том сканера  отключен.  Теперь отключите кабель от сканера и немного подождите пока я завершу распознавание текста"|RHVoice-test -p Vsevolod
...
echo "Проговариваю распознанные страницы"|RHVoice-test -p Vsevolod
RHVoice-test -i /tmp/TalkingScanner/pages.txt -p Anna
cat   /tmp/TalkingScanner/pages.txt >>/var/tmp/TalkingScanner/results.txt
echo "Проговаривание распознанных страниц завершено"|RHVoice-test -p Vsevolod
done
